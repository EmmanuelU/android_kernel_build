...
echo /tmp/mkbootimg --kernel /tmp/zImage --ramdisk /tmp/boot.img-ramdisk.gz --cmdline \"$(cat /tmp/boot.img-cmdline)\" --base 0x40400000 --ramdisk_offset 0x01400000 --output /tmp/newboot.img >> /tmp/createnewboot.sh
